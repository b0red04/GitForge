...
```bash
./scripts/build-update-tarball.sh 0.1.14
GITFORGE_BUNDLE_PATH="$(pwd)/GitForge-0.1.14-$(uname -m).tar.gz" bash scripts/install.sh
```
...
The version in `Cargo.toml`, the git tag (`v0.1.14`), and the tarball filename must all match.
