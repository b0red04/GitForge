...
```bash
./scripts/build-update-tarball.sh 0.3.2
GITFORGE_BUNDLE_PATH="$(pwd)/GitForge-0.3.2-$(uname -m).tar.gz" bash scripts/install.sh
```
...
The version in `Cargo.toml`, the git tag (`v0.3.2`), and the tarball filename must all match.
