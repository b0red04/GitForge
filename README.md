...
```bash
./scripts/build-update-tarball.sh 0.2.3
GITFORGE_BUNDLE_PATH="$(pwd)/GitForge-0.2.3-$(uname -m).tar.gz" bash scripts/install.sh
```
...
The version in `Cargo.toml`, the git tag (`v0.2.3`), and the tarball filename must all match.
