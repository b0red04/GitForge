...
```bash
./scripts/build-update-tarball.sh 0.1.8
GITFORGE_BUNDLE_PATH="$(pwd)/GitForge-0.1.8-$(uname -m).tar.gz" bash scripts/install.sh
```
...
The version in `Cargo.toml`, the git tag (`v0.1.8`), and the tarball filename must all match.
