...
impl DiffPanel {
    ...
    pub fn set_blame(&mut self, lines: Vec<BlameLine>, path: String) {
        ...
        self.blame = Some(BlameState { lines, file_path: path });
        self.highlight.cache.borrow_mut().clear();
    }
    ...
    pub fn render(
        ...
        match (repo_state, &self.diff_state, selected_commit_idx) {
            (Some(repo), Some(diff_state), Some(idx)) => {
                ...
                let commit_detail = render_commit_detail(commit, colors, border, text_color, muted, entity.clone());
                    ...
                    return diff_panel_root(surface)
                        ...
                        .child(
                            div().flex_1().flex().items_center().justify_center()
                                .child(div().text_sm().text_color(muted).child("No changes in this commit"))
                        );
                }
                ...
                for (fi, fd) in file_diffs.iter().enumerate() {
                    ...
                    let added_count = fd.lines.iter().filter(|l| l.line_type == DiffLineType::Added).count();
                    let removed_count = fd.lines.iter().filter(|l| l.line_type == DiffLineType::Removed).count();
                        ...
                        div()
                            ...
                            .child(
                                div().flex().items_center().gap_1()
                                    .child(div().text_xs().text_color(name_color)
                                        .overflow_hidden().text_ellipsis()
                                        .child(path_owned.clone()))
                                    .child(div().flex_1())
                                    .child(if added_count > 0 {
                                        div().text_xs().text_color(stats_color_added)
                                            .child(format!("+{}", added_count))
                                    } else {
                                        div()
                                    })
                                    .child(if removed_count > 0 {
                                        div().text_xs().text_color(stats_color_removed)
...
fn render_diff_content(
    ...
    if diff.is_binary {
        ...
        return div()
            ...
            .child(
                div().px_3().py_2()
                    .border_b_1().border_color(border)
                    .text_sm().font_family("monospace").text_color(muted)
            ...
            .child(
                if is_image {
                    ...
                    div().flex_1().flex().items_center().justify_center()
                        .child(div().text_sm().text_color(muted).child("Binary file (not displayed)"))
                }
            );
    }
            ...
            if first_line.content.starts_with(LFS_POINTER_HEADER) {
                ...
                return div()
                    ...
                    .child(
                        div().px_3().py_2()
                            .border_b_1().border_color(border)
                            .text_sm().font_family("monospace").text_color(text_color)
                            .child(path_label.to_string())
                    )
                    .child(
                        div().p_4().flex().flex_col().gap_2()
                            .child(div().text_sm().font_weight(FontWeight::BOLD).text_color(accent)
    ...
    let file_header = div()
        ...
        .flex().items_center()
        .child(
            div().text_sm().font_family("monospace").text_color(text_color)
                .child(path_label.to_string())
        )
        .child(div().flex_1())
        .child({
    ...
    let diff_lines = uniform_list(
        ...
        move |visible_range: Range<usize>, _window: &mut Window, _cx: &mut App| {
            ...
            for line_i in visible_range {
                ...
                let is_selected = selection_range.as_ref().map_or(false, |r| r.contains(&line_i));
                ...
                let old_num = line.old_line
                    .map(|n| format!("{:>4}", n))
                    .unwrap_or_else(|| "    ".to_string());
                let new_num = line.new_line
                    .map(|n| format!("{:>4}", n))
                    .unwrap_or_else(|| "    ".to_string());

                let content = line.content.clone();
                let use_syntax = matches!(line.line_type, DiffLineType::Context | DiffLineType::Added | DiffLineType::Removed);
                let display_content: String = content.chars().take(200).collect();

                let content_col = if use_syntax {
                    let highlighted = highlight.highlight_line(&path_for_hl, line_i, &display_content, "");
                        ...
                        div()
                            ...
                            .child(
                                div()
                                    ...
                                    .text_xs().font_family("monospace").text_color(muted)
                                    .pl_2()
                                    .child(old_num)
                            )
                            .child(
...
fn render_code_view(
    ...
    let Some(content_owned) = content.map(String::from) else {
        return div()
            .flex_1().h_full().bg(surface)
            .flex().items_center().justify_center()
            .child(div().text_sm().text_color(muted).child("No file content available"));
    };
    ...
    let code_lines = uniform_list(
        ...
        move |visible_range: Range<usize>, _window: &mut Window, _cx: &mut App| {
            ...
            for line_i in visible_range {
                let Some(line_content) = lines.get(line_i) else { continue };
                let display: String = line_content.chars().take(200).collect();

                let line_num = format!("{:>4}", line_i + 1);

                let highlighted = highlight.highlight_line(&path_owned, line_i, &display, "__code__");
...
fn render_blame_view(
    ...
    let blame_rows = uniform_list(
        ...
        move |visible_range: Range<usize>, _window: &mut Window, _cx: &mut App| {
            ...
            for line_i in visible_range {
                let Some(blame) = lines_data.get(line_i) else { continue };
                ...
                let row = div()
                    ...
                    .child(
                        div().w(px(56.0))
                            .text_xs().font_family("monospace").text_color(id_color)
                            .overflow_hidden().pl_2()
                            .child(short_id)
                    )
                    .child(
                        div().w(px(80.0))
                            .text_xs().text_color(rgba_to_hsla(cl.text_muted))
                            .overflow_hidden().pl_1()
                            .child(author)
                    )
                    .child(
                        div().flex_1()
                            .text_xs().font_family("monospace").text_color(text_color)
                            .pr_3().overflow_hidden()
                            .child(display)
                    );
